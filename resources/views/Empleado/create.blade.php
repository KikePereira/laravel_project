...
@section('content')
<div class="container">
    <form action="{{ route('empleado.store')}}" method="post" class="">
        <!-- PERSONAL -->
        <div class="row">
            <h4 class="fw-bold">Personal</h4>
             ...
             <div class="col-12 col-lg-4">
                ...
                <input type="text" class="form-control" name="dni" placeholder="DNI del empleado" value="{{old('dni')}}">
            </div>
            <!-- NOMBRE -->
            <div class="col-12 col-lg-4">
                <label for=""  class="form-label">Nombre:</label> @error('nombre')<span class="text-danger">{{$message}}</span>@enderror
                <input type="text" class="form-control" name="nombre" placeholder="Nombre del empleado" value="{{old('nombre')}}">
            </div>
            <!-- APELLIDO -->
            <div class="col-12 col-lg-4">
                <label for=""  class="form-label">Apellidos:</label> @error('apellidos')<span class="text-danger">{{$message}}</span>@enderror
                <input type="text" class="form-control" name="apellidos" placeholder="Apellido del empleado" value="{{old('apellidos')}}">
            </div>
        </div>
        <br>
        <!-- CONTACTO -->
        <div class="row">
            ...
            <div class="col-12">
                ...
                <input type="text" class="form-control" name="telefono" placeholder="Telefono del empleado" value="{{old('telefono')}}">
            </div>
            ...
            <div class="col-12 col-lg-6">
                ...
                <input type="text" class="form-control" name="email" placeholder="Correo Electronico del empleado" value="{{old('email')}}">
            </div>
            <div class="col-12 col-lg-6">
                <label for=""  class="form-label">Contraseña:</label> @error('password')<span class="text-danger">{{$message}}</span>@enderror
                <input type="text" class="form-control" name="password" placeholder="Introduce una contraseña para el empleado" value="{{old('password')}}">
            </div>
        </div>
        </div>
        <br>
